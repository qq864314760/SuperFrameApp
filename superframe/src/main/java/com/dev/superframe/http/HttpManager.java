...
//        }
    ...
    public synchronized static HttpManager getInstance() {
        if (instance == null) {
            instance = new HttpManager(BaseApp.getInstance());
        }
        return instance;
    }
        ...
        new AsyncTask<Void, Void, Exception>() {
            ...
            protected Exception doInBackground(Void... params) {
                ...
                try {
                    ...
                    Log.e(TAG, "post  AsyncTask.doInBackground  \ntry {  result = getResponseJson(...} \ncatch (Exception e) {\n" + e.getMessage() + "}");
                    return e;
                }
                Log.e(TAG, "HttpResult::" + result);
     ...
     * @param url         接口url
     * @param requestCode 请求码，类似onActivityResult中请求码，当同一activity中以实现接口方式发起多个网络请求时，请求结束后都会回调
     *                    {@link HttpManager.OnHttpResponseListener#onHttpResponse(int, String, Exception)}<br/>
     *                    在发起请求的类中可以用requestCode来区分各个请求
     * @param listener
     */
    public void upload(final List<Parameter> paramList, final String url,
        ...
        new AsyncTask<Void, Void, Exception>() {
            ...
            protected Exception doInBackground(Void... params) {
                OkHttpClient client = getHttpClient(url);
                if (client == null) {
                    return new Exception(TAG + ".upload  AsyncTask.doInBackground  client == null >> return;");
                }
                ...
                if (paramList != null) {
                    ...
                    for (Parameter p : paramList) {
                        if (p.value instanceof File) {
                            ...
                            Log.e("上传图片", ">>" + p.key + "[" + index + "]");
                            builder.addFormDataPart(p.key + "[" + index + "]", file.getName(), RequestBody.create(MediaType.parse("image/png"), file));
                            index++;
                        } else {
    ...
    private OkHttpClient getHttpClient(String url) {
        ...
        if (url.startsWith(StringUtil.URL_PREFIXs) && socketFactory != null) {
            client = new OkHttpClient.Builder()
                    .connectTimeout(15, TimeUnit.SECONDS)
                    .writeTimeout(10, TimeUnit.SECONDS)
                    .readTimeout(10, TimeUnit.SECONDS).socketFactory(socketFactory).build();
        }
