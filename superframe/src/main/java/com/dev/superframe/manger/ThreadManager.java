...
package com.dev.superframe.manger;
...
public class ThreadManager {
	...
	public Handler runThread(String name, Runnable runnable) {
		if (StringUtil.isNotEmpty(name, true) == false || runnable == null) {
			KLog.e(TAG, "runThread  StringUtil.isNotEmpty(name, true) == false || runnable == null >> return");
			return null;
		}
		...
		if (handler != null) {
			KLog.w(TAG, "handler != null >>  destroyThread(name);");
			destroyThread(name);
		}
		...
		KLog.d(TAG, "runThread  added name = " + name + "; threadMap.size() = " + threadMap.size() + "\n");
		return handler;
	}
